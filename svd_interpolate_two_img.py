...
# from ddim_with_prob_euler import EulerDiscreteScheduler_prob
            ...
            for i, t in enumerate(timesteps):
                ...
                for ii in range(4):
                    with torch.enable_grad(): 
                        ...
                        output = self.scheduler.step_multi(noise_pred_t, t, latents1,temp_cond_latents1,mask1,lambda_ts,step_i=i,lr=lr,compute_grad=True)
                        grad = output.grad
                        grads.append(grad)
                ...
                with torch.no_grad():
                    ...
                    latents = self.scheduler.step_multi(noise_pred, t, latents,temp_cond_latents,mask,lambda_ts,step_i=i,compute_grad=False).prev_sample
...
def forward_warp(frame1: np.ndarray, mask1: Optional[np.ndarray], depth1: np.ndarray,
    ...
    trans_points1 = compute_transformed_points(depth1, transformation1, transformation2, intrinsic1,
                                                    intrinsic2)
    ...
    warped_depth2 = bilinear_splatting(trans_depth1[:, :, None], mask1, trans_depth1, flow12, None,
                                            is_image=False)[0][:, :, 0]
...
def bilinear_interpolation(frame2: np.ndarray, mask2: Optional[np.ndarray], flow12: np.ndarray,
...
def warp_images(interpolated_poses,src1,src2,save_path):
    ...
    return image_o,image_o2,masks,cond_image


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--lr',type=float,default=0.1)
    parser.add_argument('--src1',type=int,default='7' )
    parser.add_argument('--src2',type=int,default='18')
    parser.add_argument('--scene',type=str,default='playground')
    parser.add_argument('--dataset',type=str,default='tat')
    ...
    if args.dataset == 'dtu':
        ...
        R2 = Rs[src2-1]
        T2 = ts[src2-1].reshape(3)
        K = Ks[src1-1]
        
        
        K[0,0] = K[0,0]/504*1024; K[1,1] = K[1,1]/274*576; K[0,2] = K[0,2]/504*1024; K[1,2] = K[1,2]/274*576
        K = K.astype(np.float32)
    pose_start = np.eye(4)
    pose_start[:3, :3] = R1
    pose_start[:3, 3] = T1
    ...
    image_o,image_o2,masks, cond_image = warp_images(interpolated_poses,src1,src2,save_path)
    ...
    frames = pipe([image_o],temp_cond=cond_image+[image_o2],mask = masks,lambda_ts=lambda_ts,lr=lr,num_frames=25, decode_chunk_size=8,num_inference_steps=100).frames[0]
   
    output_path = save_path+'_out'
