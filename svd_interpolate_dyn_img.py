...
def bilinear_splatting(frame1: np.ndarray, mask1: Optional[np.ndarray], depth1: np.ndarray,
    ...
    sat_depth1 = np.clip(depth1, a_min=0, a_max=10000)
...
def save_warped_image(save_path,images_lists,depth_lists,num_frames, degrees_per_frame,major_radius,minor_radius,inverse=False,up=False):
    ...
    pose_paths = [pp.replace('images','camera_params').replace('png','npz') for pp in images_lists]
...
def svd_render(image_o,masks, cond_image,warped_img_path,output_path,lambda_ts,lr,weight_clamp):
    pipe = StableVideoDiffusionPipeline.from_pretrained("stabilityai/stable-video-diffusion-img2vid-xt", torch_dtype=torch.float16, variant="fp16")
    pipe.to("cuda")
...
def search_hypers(sigmas,save_path):
    ...
    v1_list = np.arange(50, 2001, 50)
    v2_list = np.arange(10, 201, 10)
                ...
                for sigma in sigmas:
                    ...
                    for tau in range(25):
                        if tau not in index_list:
                            ...
                            Q = v2 * abs((tau_-tau_p)**2) - 2*v1*sigma_n/(2*math.pi)**0.5
                            k = 0.8
                            b = -0.2
...
if __name__ == "__main__":
    ...
    image_path= os.path.join(args.folder_path,'images')
    depth_path = os.path.join(args.folder_path,'depth')
    images_lists = []
    depth_lists = []

    for i in range(0, 25, 1):
        images_lists.append(os.path.join(image_path,'frame_'+str(i).zfill(6)+'.png'))
        depth_lists.append(os.path.join(depth_path,'frame_'+str(i).zfill(6)+'.npy'))
